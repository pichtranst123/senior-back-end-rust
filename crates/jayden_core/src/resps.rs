...
impl Web {
  pub fn new<'a>(
    ...
    (code, Json(Web { code, message: message.to_string(), data: json!(&data), error: error.to_string() }))
      .into_response()
  }
  ...
  pub fn no_content<'a>(message: impl ToString, data: impl Serialize + Deserialize<'a>) -> Response {
  ...
  pub fn unprocessable_entity(message: impl ToString, error: impl ToString) -> Response {
    Web::new(StatusCode::UNPROCESSABLE_ENTITY, message, json!(null), error)
  }
  ...
  pub fn internal_error(message: impl ToString, error: impl ToString) -> Response {
    Web::new(StatusCode::INTERNAL_SERVER_ERROR, message, json!(null), error)
  }
}
